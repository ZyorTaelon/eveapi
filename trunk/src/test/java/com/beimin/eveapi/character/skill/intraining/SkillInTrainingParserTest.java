...
public class SkillInTrainingParserTest {
	...
	public void skillInTrainingParser() throws IOException, SAXException,
		...
		InputStream input = SkillInTrainingParserTest.class
				.getResourceAsStream("/character/SkillInTraining.xml");
		...
		assertEquals(currentTQDate.toString(), response.getCurrentTQDate()
				.toString());
		calendar.set(2008, 7, 17, 15, 29, 44); // 2008-08-17 15:29:44
		Date endDate = calendar.getTime();
		assertEquals(endDate.toString(), response.getTrainingEndDate()
				.toString());
		calendar.set(2008, 7, 15, 4, 1, 16); // 2008-08-15 04:01:16
		Date startDate = calendar.getTime();
		assertEquals(startDate.toString(), response.getTrainingStartDate()
				.toString());
