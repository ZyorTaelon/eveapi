...
public class BalanceParser extends AbstractApiParser<BalanceResponse> {
	protected static final String ACCOUNTBALANCE_URL = "/AccountBalance.xml.aspx";

	public BalanceParser() {
		super(BalanceResponse.class, 1, ACCOUNTBALANCE_URL);
	}
	...
	public BalanceResponse getAccountBalance(ApiAuth auth, boolean corporation) throws IOException, SAXException {
		if (corporation)
			return getResponse(auth, Path.CORP);
		return getResponse(auth, Path.CHARACTER);
	}

	public static BalanceParser getInstance() {
