...
public class CertificateTreeParserTest extends NoAuthParserTest {
    ...
    public void getResponse() throws ApiException {
        ...
        final Set<CertificateCategory> certificateCategories = response.getAll();
        assertEquals("Wrong number of Certificate Categories", 11, certificateCategories.size());
        final CertificateCategory certificateCategory = certificateCategories.iterator().next();
        assertEquals(3, certificateCategory.getCategoryID());
        assertEquals("Core", certificateCategory.getCategoryName());
        final List<CertificateClass> certificateClasses = certificateCategory.getCertificateClasses();
        assertEquals("Wrong number of Certificate Classes", 6, certificateClasses.size());
        final CertificateClass certificateClass = certificateClasses.iterator().next();
        assertEquals(2, certificateClass.getClassID());
        assertEquals("Core Fitting", certificateClass.getClassName());
        final List<Certificate> certificates = certificateClass.getCertificates();
        ...
        final List<RequiredSkill> requiredSkills = certificate.getRequiredSkills();
        assertEquals("Wrong number of required skills", 5, requiredSkills.size());
        final RequiredSkill requiredSkill = requiredSkills.iterator().next();
        assertEquals(3318, requiredSkill.getTypeID());
        assertEquals(4, requiredSkill.getLevel());
        final List<RequiredCertificate> requiredCertificates = certificate.getRequiredCertificates();
        assertEquals("Wrong number of required certificates", 1, requiredCertificates.size());
        final RequiredCertificate requiredCertificate = requiredCertificates.iterator().next();
