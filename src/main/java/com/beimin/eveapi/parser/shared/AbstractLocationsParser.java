...
public abstract class AbstractLocationsParser extends AbstractListParser<LocationsHandler, LocationsResponse, Location> {
    protected AbstractLocationsParser(final ApiPath path) {
        super(LocationsResponse.class, 2, path, ApiPage.LOCATIONS, LocationsHandler.class);
    }

    public LocationsResponse getResponse(final ApiAuth auth, final List<Long> itemIDs) throws ApiException {
        final StringBuilder idList = new StringBuilder();
        for (final Iterator<Long> iterator = itemIDs.iterator(); iterator.hasNext();) {
            idList.append(iterator.next());
            if (iterator.hasNext()) {
                idList.append(",");
            }
        }
